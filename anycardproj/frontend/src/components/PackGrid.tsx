...
import { getAllSets, openPack } from "../api/cards";
...
const PackGrid = ({
  ...
  function WebGLContextHandler() {
    ...
    useEffect(() => {
      ...
      const handleContextRestored = () => {
        console.log("WebGL context restored in PackGrid");
  ...
  const handlePackClick = async (index: number) => {
    ...
    if (newCount === 1) {
      ...
      if (pack && user?.uid) {
        try {
          const result = await openPack(user.uid, pack.name, "cards");
  ...
  if (isLoading) {
    return (
      <div className={`flex items-center justify-center ${className}`}>
        <div className="text-lg">Loading packs...</div>
      </div>
    );
  }
